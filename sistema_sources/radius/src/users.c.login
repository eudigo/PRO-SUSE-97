...
static text *seleuser = (text *) "SELECT userpass FROM users WHERE username = :username";
...
char userpass[TAM_MAX_AUTH_PASS+1]; /* Senha do Usuario */
...
static OCIDefine *defnp = (OCIDefine *) 0; /* Define (Output Variable) */
...
orauser_open(void)
{
   ...
   log_err("Conectando ao Servidor Oracle\n");
   ...
   log_err("Iniciando Sessao de Usuario: scott/tiger\n");
   ...
   log_err("Alocando Handle de Statement\n");
   ...
   log_err("Preparando Execucao do Comando\n");
   ...
   log_err("Bind the Input Variable\n");
   ...
   if (status = OCIBindByName(stmthp, &bindp, errhp, (text *) ":username", strlen(":username"), (ub1 *) username, TAM_MAX_AUTH_NAME, SQLT_AFC, (dvoid *) 0, (ub2 *) 0, (ub2 *) 0, (ub4) 0, (ub4 *) 0, OCI_DEFAULT))
{
   ...
   log_err("Define the Output Variable\n");

   /* Define the output variable (password) */
   /* O Handle de Define e' alocado automaticamente, e vinculado a Statement */

   if (status = OCIDefineByPos(stmthp, &defnp, errhp, 1, (dvoid *) userpass, TAM_MAX_AUTH_PASS+1, SQLT_STR, (dvoid *) 0, (ub2 *) 0, (ub2 *) 0, OCI_DEFAULT))
{
   orauser_error(errhp, status);
   orauser_close();
   return (-1);
 ...
 */
   ...
   for (;;) {
      ...
      log_err("Executando o comando SQL\n");
      ...
      if (status == OCI_NO_DATA) { /* Se Usuario nao Encontrado */
         ...
         log_err("Status do Comando SQL = OCI_NO_DATA. Usuario: %s Tamanho: %d\n",username,strlen(username));
         ...
         if (db_index == 0) { /* E e' o nome que consta na requisicao */
            ...
            log_err("Comando SELECT nao encontrou o Usuario: %s Vai tentar o usuario DEFAULT\n",username);
      ...
      /*
       * Parse the check values (A MODIFICAR)
       *
   ...
   log_err("OK: Check Values Parsed\n");
...
*/
      ...
      sprintf(pair->name,"Login-IP-Host");
      pair->attribute = PW_LOGIN_HOST;
      pair->type = PW_TYPE_IPADDR;
      pair->lvalue = get_ipaddr("200.240.31.80"); 
      ...
      sprintf(pair->name,"Login-Service");
      pair->attribute = PW_LOGIN_SERVICE;
      pair->type = PW_TYPE_INTEGER;
      pair->lvalue = PW_TELNET;
      ...
      log_err("OK: Reply Values Parsed\n");
